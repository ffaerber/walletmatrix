...
// Testnet detection is based on the registry's faucets field.

export const CHAIN_IDS: number[] = [
...
function buildChain(numId: number, entry: RegistryEntry | null): Chain {
...
export async function initChains(): Promise<void> {
  ...
  const loaded = entries
    ...
    .map(({ numId, entry }) => buildChain(numId, entry));
